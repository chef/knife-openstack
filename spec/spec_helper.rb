...
def find_instance_id(instance_name, file)
  file.lines.each do |line|
    return line.to_s.split(' ')[2].strip if line.include?(instance_name.to_s)
  end
end
...
def is_config_present
  ...
  openstack_config = YAML.safe_load(File.read(File.expand_path('../integration/config/environment.yml', __FILE__))) if config_file_exist
