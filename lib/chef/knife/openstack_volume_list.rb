      ...
      def run
        ...
        begin
          ...
          vol_list.each do |volume|
            ...
            volume_list << ui.color(volume.status, status_color(volume.status))
            volume_list << volume.availability_zone
            if volume.attachments.size > 0
              volume_list << volume.attachments[0]["device"]
              volume_list << get_server_name_by_id(volume.attachments[0]["server_id"]) if 'Cinder' == target
              volume_list << get_server_name_by_id(volume.attachments[0]["serverId"]) if 'Nova' == target
            else
              volume_list << ""
              volume_list << ""
            end
